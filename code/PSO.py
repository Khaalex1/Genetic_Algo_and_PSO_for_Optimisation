...
July    1999
    ...
    def optimization_algorithm(self, n_iter, n_informants):
        ...
        for n in range(n_iter):
            ...
            if ((n_iter >= 2000) and ((n % 100) == 0 or n == n_iter-1)) or ((n_iter < 2000) and ((n % 30) == 0 or n == n_iter-1)):
                #print('---'*10)
                print('Iter {} - Best fitness value = {}'.format(n, np.round(self.__fitness.value(best_position), 6)[0]))
            # Update the history
        ...
        print("---")
        print(f"1 simulation of {n_iter} iterations report:")
        print("Best individual overall = ", np.around(best_position, 6))
        print("Best fitness overall = ", np.around(self.__fitness.value(best_position), 6))
        #self.result_curve()
...
def repeted_test(fitness, n_dim, boundary, n_test, n_particles, opt, alpha, beta, gamma, delta, eps, iter_max, n_informants):
    ...
    print("======================================================")
    print(f"{n_test} simulations of {iter_max} iterations report:")
    print("Average fitness = ", np.mean(best_fit))
    print("Average fitness Std = ", np.std(best_fit))
    print("Best fitness overall = ", np.around(np.min(best_fit), 6))
    # print('-------'*5)
