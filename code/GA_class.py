...
def culling(A, fitness_func, number = 1):
    ...
    :param number: number to eliminate
    :return: pop without eliminated indiv
    """
    pop = copy.deepcopy(A)
...
def Fitness(name = "sphere", n_dim = 10, boundary = [-10, 10]):
    """
    Fitness function which take into account all studied fitnesses
    :param name: 'sphere', 'rastrigin', 'rosenbrock', 'griewank'
    :param n_dim: number of dimenion of function
    :param boundary: 2 index list : [min, max]
    :return: the fitness class based on BenchmarkFunctions
    """
...
class GA:
    """
    GA class to do experiment
    """
    def __init__(self, nb_genes, initial_pop_size, bound_min, bound_max, fitness_name, elites = 0, cull = 0):
        """
        GA constructor
        :param nb_genes: number of dim of fitness
        :param initial_pop_size: number of individuals
        :param bound_min: boundary min
        :param bound_max: boundary max
        :param fitness_name: fitness name cf Fitness
        :param elites: number of elites
        :param cull: number of eliminated individuals
        """
    ...
    def selection_init(self, number, name = "wheel", tournament_size = None, proba = None):
        ...
        :param number: number of selected parents
        :param name: type of selection, cf Selection
        :param tournament_size: tournament size
        :param proba: winner of tournament proba to get selected
        :return:
        """
    ...
    def crossover_init(self, child_nb, name = "uniform", alpha = 0, rate = 1):
        ...
        :param child_nb: number of children to produce
        :param name: name of crossover cf Crossover
        :param alpha: hyperparameter of the crossover cf Crossover
    ...
    def mutation_init(self, name = "shrink", width = 0, rate = 0):
        """
        Initialize Mutation class
        :param name: name of muttion cf Mutation
        :param width: width of uniform mutation
    ...
    def simulation(self,  nb_gen = 500, precision = 4, printer = True, thresh_reset = 150):
        """
        Do a single run simulation
        :param nb_gen: number of generations/iterations to evaluate
        :param precision: n -> results rounded to 1e(-n)
        :param printer: if True print results every 10 gen
        :param thresh_reset: If fitness is above thresh reset at every quarter of nb_gen, population is resetted
        :return:
        """
        ...
        if self.mutation.name == "cmaes":
            ...
            for i in range(1, nb_gen+1):
                ...
                # mutation
                children = self.mutation.apply(children, width = 0)
                # elitism & culling
    ...
    def multiple_runs(self, nb_runs=10, nb_gen=1000, precision=4,printer=False, thresh_reset = 150):
        """
        Do multiple runs using simulation function
        :param nb_runs: number of runs
        :param nb_gen: /
        :param precision: /
        :param printer: /
        :param thresh_reset: /
...
if __name__ == "__main__":
    experience = GA(nb_genes=10, initial_pop_size=40, bound_min=-100, bound_max=100, fitness_name="rastrigin", elites = 5, cull = 5)
